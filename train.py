...
def main():
    ...
    mini = few_shot_dataset('L:\桌面文件\假期资料\小样本学习问题\MAML元学习不平衡分类pytorch实现\训练数据集', mode='zhuzhou_bearing_meta_danyi_train', batchsz=args.task, n_way=args.n_way, k_shot=args.k_spt, k_query=args.k_qry, resize=1200, train_test_split=args.train_test_radio, selected_train=True)
    mini_test = few_shot_dataset('L:\桌面文件\假期资料\小样本学习问题\MAML元学习不平衡分类pytorch实现\训练数据集', mode='zhuzhou_bearing_meta_danyi_train', batchsz=args.task_test, n_way=args.n_way_test, k_shot=args.k_spt_test, k_query=args.k_qry_test, resize=1200, train_test_split=args.train_test_radio, selected_train=False)
